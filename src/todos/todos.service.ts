...
export class TodosService {
    ...
    async getUserCompletedTodoList(
        ...
        const todoStatus: TodoStatus[] = [TodoStatus.COMPLETED];

        const mainQuery = this.todosRepository
            .createQueryBuilder('todo')
            .leftJoinAndSelect('todo.manager', 'manager')
            .where('todo.status IN (:...status)', { status: todoStatus })
            .andWhere('todo.manager.id = :userId', { userId })
            .orderBy('todo.completedAt', 'ASC')
            .addOrderBy('todo.id', 'DESC');

        const totalCount = await mainQuery.getCount();

        const todoList = await mainQuery
    ...
    async getTodoListForUserId(
        ...
        perPage: number = 20,
        userId: number,
        todoStatusOption: "all_uncompleted" | "all_completed" | "idea" | "uncompleted" | "complete" | "entry" | "all"
    ): Promise<{
        todoList: TodosModel[],
        totalCount: number,
        perPage: number,
        ...
        const usersEmailInfo = [userEmail.user_email];

        let todoStatus: TodoStatus[];

        switch (todoStatusOption) {
            case "all_uncompleted":
                todoStatus = [TodoStatus.IDEA, TodoStatus.READY, TodoStatus.PROGRESS, TodoStatus.TESTING];
                break;
            case "all_completed":
                todoStatus = [TodoStatus.COMPLETED];
                break;
            case "uncompleted":
                todoStatus = [TodoStatus.READY, TodoStatus.PROGRESS, TodoStatus.TESTING];
                break;
            case "complete":
                todoStatus = [TodoStatus.COMPLETED];
                break;
        ...
        if (todoStatus.length > 0 && todoStatus.length < Object.values(TodoStatus).length) {
            mainQuery = mainQuery.andWhere('todo.status IN (:...status)', { status: todoStatus });
        }

        // 여기에 추가적인 정렬 로직을 넣을 수 있습니다.
